...
/**
 * PlayerListener is the class used to register the event handlers needed for
 * this plugin's operation.
 * 
 * @author mstiles92
 */
public class PlayerListener implements Listener {
	...
	public void onPlayerDeath(PlayerDeathEvent e) {
		if (plugin.getConfig().getBoolean("Enabled")
				&& !(e.getEntity().hasPermission("deathban.ban.exempt"))) {
	...
	public void onPlayerJoin(PlayerJoinEvent e) {
		if (HardcoreDeathBan.getInstance().getUpdateChecker()
				.isUpdateAvailable()
				&& e.getPlayer().hasPermission("deathban.receivealerts")) {
			e.getPlayer()
					.sendMessage(
							ChatColor.GREEN
									+ "[HardcoreDeathBan] New version available! See http://dev.bukkit.org/bukkit-plugins/hardcoredeathban/ for more information.");
			e.getPlayer().sendMessage(
					ChatColor.GREEN
							+ "[HardcoreDeathBan] Current version: "
							+ ChatColor.BLUE
							+ plugin.getDescription().getVersion()
							+ ChatColor.GREEN
							+ ", New version: "
							+ ChatColor.BLUE
							+ HardcoreDeathBan.getInstance().getUpdateChecker()
									.getNewVersion());
		}
	}
		...
		if (plugin.getConfig().getBoolean("Enabled")) {
			if (plugin.bans.checkPlayerIsBanned(e.getUniqueId())) {
				if (plugin.credits.getPlayerCredits(e.getName()) < 1) {
					...
					final String s = plugin.getConfig().getString(
							"Early-Message");
					e.disallow(
							Result.KICK_BANNED,
							plugin.replaceVariables(s, e.getName(),
									e.getUniqueId()));
				} else {
					plugin.log("Banned player redeemed 1 revival credit upon login: "
							+ e.getName());
				...
				final int startingCredits = plugin.getConfig().getInt(
						"Starting-Credits");
				plugin.log("New player recieved "
						+ Integer.toString(startingCredits)
						+ " revival credits upon their first login: "
						+ e.getName()); // TODO: remove give of credits on first
										// join
