...
/**
 * DeathClasses is a class used to store information about various Death Classes
 * that will be used to control the amount of time a player is banned, how many
 * Revival Credits they start out with, and any other various attributes
 * assigned to players by their death class.
 * 
 * @author mstiles92
 */
public class DeathClasses {
	 ...
	 * @param filename
	 *            the name of the file to save to disk
	 * @throws Exception
	 *             if there is an error while opening or creating the file
	 */
	public DeathClasses(HardcoreDeathBan plugin, String filename)
			throws Exception {
		this.plugin = plugin;
		load(filename);
	}
	 ...
	 * 
	 * @param deathClass
	 *            the death class to look up
	 * @return the number of starting credits the player would recieve, or -1 if
	 *         their class could not be found
	 */
	 ...
	 *         unbanned if they had this class, or null if the death class could
	 *         not be found, or the ban time was not able to be parsed
	 */
	public Calendar getClassUnbanTime(String deathClass) {
		if (deathClass == null)
			return null;
		final String unbanTime = config.getString("Classes." + deathClass
				+ ".Ban-Time");
		if (unbanTime == null)
			return null;
		try {
	 ...
	 * @return the name of the death class if the player has one, null if they
	 *         do not
	 */
	 ...
	 * @param player
	 *            the player to check
	 * @return a Calendar object representing the time that the player will be
	 *         unbanned, or null if they do not have a death class
	 */
	...
	public void save() {
		try {
			...
			plugin.getLogger().warning(
					ChatColor.RED
							+ "Error occurred while saving bans config file.");
		}
	}
}
