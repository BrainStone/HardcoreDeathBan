...
public class Bans {
	...
	public Bans(HardcoreDeathBan plugin, String filename) throws Exception {
		...
		deathClasses = plugin.getConfig()
				.getConfigurationSection("Death-Classes").getKeys(false);
		if (deathClasses.size() == 0) {
			plugin.log("No death classes found.");
		} else {
			for (final String s : deathClasses) {
	...
	/**
	 * Ban a player for their default time, taking possible death classes into
	 * account.
	 * 
	 * @param player
	 *            the player to ban
	...
	public void banPlayer(UUID player) {
		...
		if (p != null) {
			for (final String s : deathClasses) {
				final Permission perm = new Permission("deathban.class." + s);
				perm.setDefault(PermissionDefault.FALSE);
				if (p.hasPermission(perm)) {
					banPlayer(
							player,
							plugin.getConfig().getString(
									"Death-Classes." + s + ".Ban-Time"));
	 ...
	 *            the player to ban
	 * @param time
	 *            the amount of time the player will be banned
	 */
				...
				if (!p.hasPermission("deathban.ban.exempt")) {
					...
					plugin.getServer()
							...
							.scheduleSyncDelayedTask(plugin,
									new KickRunnable(plugin, player),
									plugin.getConfig().getInt("Tick-Delay"));
				}
	 ...
	 * 
	 * @param player
	 *            name of the player to be unbanned
	 */
	public void unbanPlayer(UUID player) {
		plugin.log("Player unbanned: " + player);
		config.set(player.toString(), null);

		final ImprovedOfflinePlayer offlinePlayer = new ImprovedOfflinePlayer(
				player);
