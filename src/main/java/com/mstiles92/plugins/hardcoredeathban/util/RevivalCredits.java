...
public class RevivalCredits {
	 ...
	 */
	public RevivalCredits(HardcoreDeathBan plugin, String filename)
			throws Exception {
		this.plugin = plugin;
		load(filename);
	}
	...
	public int getPlayerCredits(String player) {
		if (!config.contains(player.toLowerCase())) {
			config.set(player.toLowerCase(),
					plugin.getConfig().getInt("Starting-Credits"));
			save();
		}
	...
	public void giveAllPlayersCredits(int amount) {
		for (final String player : config.getKeys(false)) {
	 ...
	 * 
	 * @param player
	 *            the player to give the credits to
	 * @param amount
	 *            the amount of credits to give
	 */
	public void givePlayerCredits(String player, int amount) {
		...
		config.set(player.toLowerCase(),
				amount + config.getInt(player.toLowerCase()));
		save();
	}
	...
	public void resetAllPlayersCredits() {
		final int startingAmount = plugin.getConfig()
				.getInt("Starting-Credits");
		for (final String player : config.getKeys(false)) {
	...
	public void save() {
		try {
			...
			plugin.getLogger()
					.warning(
							ChatColor.RED
									+ "Error occurred while saving credits config file.");
		}
	}
	 ...
	 * 
	 * @param player
	 *            the player to set the credits for
	 * @param amount
	 *            the amount of credits the player should have
	 */
