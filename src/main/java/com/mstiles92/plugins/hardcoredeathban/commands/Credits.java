...
/**
 * Credits is the CommandExecutor that handles all commands dealing with credits
 * for this plugin.
 * 
 * @author mstiles92
 */
public class Credits implements CommandHandler {
		...
		if (args.getArgs().length < 1) {
			if (args.isPlayer()) {
				args.getPlayer().sendMessage(
						tag
								+ "Revival credits: "
								+ plugin.credits.getPlayerCredits(args
										.getPlayer().getName()));
			} else {
				args.getSender()
						.sendMessage(
								tag
										+ ChatColor.RED
										+ "This command can not be run from the console unless a player is specified.");
			}
		} else {
		...
		if (args.getArgs().length < 2) {
			args.getSender()
					.sendMessage(
							tag
									+ ChatColor.RED
									+ "You must specify both a player and an amount to give that player.");
			return;
		}

		try {
			if (Integer.parseInt(args.getArgs()[1]) < 1)
				throw new NumberFormatException();
			plugin.credits.givePlayerCredits(args.getArgs()[0],
					Integer.parseInt(args.getArgs()[1]));
			args.getSender().sendMessage(
					tag + "You have successfully given " + args.getArgs()[0]
							+ " " + args.getArgs()[1] + " revival credits.");
		} catch (final NumberFormatException e) {
			args.getSender()
					.sendMessage(
							tag
									+ ChatColor.RED
									+ "The amount must be specified as a positive integer value.");
		}
	}
	...
	public void send(Arguments args) {
		if (args.getArgs().length < 3) {
			args.getPlayer()
					.sendMessage(
							tag
									+ ChatColor.RED
									+ "You must specify both a player and an amount to send that player.");
			return;
		}
		...
		try {
			if (Integer.parseInt(args.getArgs()[1]) < 1)
				throw new NumberFormatException();
			if (plugin.credits.getPlayerCredits(args.getPlayer().getName()) >= Integer
					.parseInt(args.getArgs()[1])) {
				plugin.credits.givePlayerCredits(args.getPlayer().getName(),
						Integer.parseInt(args.getArgs()[1]) * -1);
				plugin.credits.givePlayerCredits(args.getArgs()[0],
						Integer.parseInt(args.getArgs()[1]));
				args.getPlayer()
						.sendMessage(
								tag + "You have successfully sent "
										+ args.getArgs()[0] + " "
										+ args.getArgs()[1]
										+ " revival credits.");
			} else {
				args.getPlayer().sendMessage(
						tag + ChatColor.RED
								+ "You do not have enough revival credits.");
			}
		} catch (final NumberFormatException e) {
			args.getPlayer()
					.sendMessage(
	...
	public void take(Arguments args) {
		...
		try {
			...
			plugin.credits.givePlayerCredits(args.getArgs()[0],
					Integer.parseInt(args.getArgs()[1]) * -1);
			args.getSender().sendMessage(
					tag + "You have successfully taken " + args.getArgs()[1]
							+ " revival credits from " + args.getArgs()[0]
							+ ".");
		} catch (final NumberFormatException e) {
			args.getSender()
					.sendMessage(
