...
/**
 * Deathban is the CommandExecutor that handles all commands dealing with bans
 * for this plugin.
 * 
 * @author mstiles92
 */
public class Deathban implements CommandHandler {
	...
	public void deathban(Arguments args) {
		args.getSender().sendMessage(
				ChatColor.GREEN + "====[HardcoreDeathBan Help]====");
		args.getSender().sendMessage(
				ChatColor.GREEN + "<x> " + ChatColor.DARK_GREEN
						+ "specifies a required parameter, while "
						+ ChatColor.GREEN + "[x] " + ChatColor.DARK_GREEN
						+ "is an optional parameter.");
		args.getSender().sendMessage(
				ChatColor.GREEN + "hdb" + ChatColor.DARK_GREEN + " or "
						+ ChatColor.GREEN + "db " + ChatColor.DARK_GREEN
						+ "may be used in place of " + ChatColor.GREEN
						+ "deathban" + ChatColor.DARK_GREEN
						+ " in the commands below.");
		args.getSender().sendMessage(
				ChatColor.GREEN + "/deathban enable " + ChatColor.DARK_GREEN
						+ "Enable the plugin server-wide.");
		args.getSender().sendMessage(
				ChatColor.GREEN + "/deathban disable " + ChatColor.DARK_GREEN
						+ "Disable the plugin server-wide.");
		args.getSender()
				.sendMessage(
						ChatColor.GREEN
								+ "/deathban ban <player> [time] "
								+ ChatColor.DARK_GREEN
								+ "Manually ban a player. Uses default time value if none specified.");
		args.getSender().sendMessage(
				ChatColor.GREEN + "/deathban unban <player> "
						+ ChatColor.DARK_GREEN
						+ "Manually unban a banned player.");
		args.getSender().sendMessage(
				ChatColor.GREEN + "/deathban status <player> "
						+ ChatColor.DARK_GREEN
						+ "Check the ban status of a player.");
		args.getSender()
				.sendMessage(
						ChatColor.GREEN
								+ "/credits [player] "
								+ ChatColor.DARK_GREEN
								+ "Check your own or another player's revival credits.");
		args.getSender()
				.sendMessage(
						ChatColor.GREEN
								+ "/credits send <player> <amount> "
								+ ChatColor.DARK_GREEN
								+ "Send some of your own revival credits to another player.");
		args.getSender().sendMessage(
				ChatColor.GREEN + "/credits give <player> <amount> "
						+ ChatColor.DARK_GREEN
						+ "Give a player a certain amount of revival credits.");
		args.getSender()
				.sendMessage(
						ChatColor.GREEN
